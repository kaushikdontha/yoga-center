...
[
  ...
  tempDir,
  cacheDir,
  ...CATEGORIES.map((cat) => path.join(eventsDir, cat)),
].forEach((dir) => {
  try {
...
const processImage = async (inputPath, outputPath, size) => {
  try {
    ...
    console.log(
      `Generated ${size.width}x${size.height} variant: ${outputPath}`
    );
    return true;
  } catch (error) {
  ...
  try {
    for (const [size, dimensions] of Object.entries(IMAGE_SIZES)) {
      ...
      if (await processImage(originalPath, variantPath, dimensions)) {
        variants[size] = path
          .relative(uploadsDir, variantPath)
          .replace(/\\/g, "/");
      }
    }
...
mongoose
  .connect(process.env.MONGODB_URI || "mongodb://localhost:27017/yoga-center", {
...
const dedupeMiddleware = async (req, res, next) => {
  ...
  if (inFlightRequests.has(key)) {
    console.log(
      "Duplicate request detected - waiting for original request:",
      key
    );
    try {
...
app.get(
  "/api/health",
  [authenticateToken, dedupeMiddleware, cacheMiddleware(5)],
  (req, res) => {
...
app.get(
  "/api/events",
  [authenticateToken, dedupeMiddleware, cacheMiddleware(30)],
  async (req, res, next) => {
...
app.get(
  "/api/events/:id",
  [authenticateToken, cacheMiddleware(30)],
  async (req, res) => {
    try {
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      // Invalidate cache
      const cacheKeys = requestCache
        .keys()
        .filter((key) => key.includes(`/api/events/${req.params.id}`));
...
app.post(
  "/api/contact",
  [apiLimiter, express.json({ limit: "10kb" })],
  async (req, res) => {
    try {
